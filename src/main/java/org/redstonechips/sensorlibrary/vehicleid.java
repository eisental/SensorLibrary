         		...
         		for (byte i=0; i<args.length; i++) {
         			if ((args[i].toUpperCase().startsWith("D{") || args[i].toUpperCase().startsWith("DIST{")) && args[i].endsWith("}")) {
         				...
         				try {                            
         		            ...
         		            if (distance==0 || distance>maxIdDistance) return error("Maximum distance exceeded: " + args[i] + ". The maximum distance is " + maxIdDistance + ". Set vehicleid.maxdistance in preferences.yml to extend the maximum distance.");
